...
public class Stemmer {
	...
	public Stemmer() {
		...
		i = 0;
		i_end = 0;
		stemmed_words = new ArrayList<>();
	}

	public List<String> getStemmed_words() {
	...
	public void stemming(String string, List<String> stopwords){
		
		//The \\W means not an alphanumeric character.
		String[] contents = string.split("\\W+");
		for (int j = 0; j < contents.length; j++) {
			Stemmer stem = new Stemmer();
			//Remove numbers and other special characters
			if(contents[j].isEmpty() || !Character.isLetter(contents[j].charAt(0)))
				continue;
			
			//remove stop words
			if(stopwords.contains(contents[j].toLowerCase()))
				continue;
			
			//System.out.println("valid tokens: " + contents[j]);
			for(int i = 0; i < contents[j].length(); i++)
				stem.add(contents[j].toLowerCase().charAt(i));
			
			//stemming
			stem.stem();
			//System.out.println("Stemmed words: " + stem.toString());
			stemmed_words.add(stem.toString());
		}
